...
#'
#' @param geno Full path to the plink files, not including the extension.
#' For example, for /home/myuser/mygeno.(bed/bim/fam) files, the geno value
#' should be the common prefix "/home/myuser/mygeno".
#' @param covs A covariates file with the minimal information about samples
#' @param predictor
#' @param pheno
#'
#' @return
...
#'
#' @examples
getHandlerToGenotypeData = function(geno,
                                    covs,
                                    predictor,
                                    id,
                                    fid,
  ...
  cat("Creating handler with genotype",geno,
      ...
      predictor,"and phenotype file",pheno,
      ". \nEverything will be written at",wp,"\n")
...
#' Title Verifies whether the handler has data created
#'
#' @param h
#' @param level
#'
...
print.genohandler = function(handler){
  ...
  if(!is.null(handler$father)){
    cat("The handler has a father handler\n")
    print(handler$father)
  }
}
...
#'
#' @examples
getPartitionsFromHandler = function(genoHandler,
                                    path2plink,
                                    workPath=NULL,
                                    how="k-fold cv",
                                    k=10,
  ...
  genoHandler = genDataFileNames(genoHandler=genoHandler,
...
getPartitionsFromMLData = function(mldatafile,
  ...
  trainData = fread(mldatafile,stringsAsFactors=F,header=T,sep="\t")
  handler = NULL
  handler$plan = how
  handler$mldatafile = mldatafile

  if(!is.null(workPath)){
    mldatafile = pate0(workPath,"/",basename(mldatafile))
  }

  library(caret)
  pred <- trainData$PHENO

  if(how == "holdout"){
    hoFold = createDataPartition(y=trainData$PHENO,p=p)
    handler$folds = hoFold
    handler$nfolds = 1
    handler$trainFolds = NULL
    handler$testFolds = NULL
    handler$trainFolds$train1 = hoFold$Resample1
    handler$testFolds$test1 = (1:nrow(trainData))[-hoFold$Resample1]
    handler$trainFolds$train1file = paste0(mldatafile,"_train1.tsv")
    fwrite(trainData[handler$testFolds$train1,],handler$trainFolds$train1file)
    handler$testFolds$test1file = paste0(mldatafile,"_test1.tsv")
    fwrite(trainData[handler$testFolds$test1,],handler$testFolds$test1file)


  }else{
    folds <- createFolds(y=trainData$PHENO, k=k, list = TRUE, returnTrain = FALSE)
    handler$folds = folds
    handler$nfolds = k
    ...
    for (i in 1:handler$nfolds){
      trainFolds[[paste0("train",i)]] = NULL
      trainFolds[[paste0("train",i)]] = (1:nrow(trainData))[-folds[[names(folds[i])]]]
      testFolds[[paste0("test",i)]] = NULL
      testFolds[[paste0("test",i)]] = folds[[names(folds[i])]]

      trainFoldFiles[[paste0("train",i)]] = NULL
      trainFoldFiles[[paste0("train",i)]] = paste0(mldatafile,"_train",i,".tsv")
      fwrite(trainData[trainFolds[[paste0("train",i)]],],
             trainFoldFiles[[paste0("train",i)]])
      testFoldFiles[[paste0("test",i)]] = NULL
      testFoldFiles[[paste0("test",i)]] = paste0(mldatafile,"_test",i,".tsv")
      fwrite(trainData[testFolds[[paste0("test",i)]],],
             testFoldFiles[[paste0("test",i)]])
    }
    handler$trainFolds = trainFolds
    handler$testFolds = testFolds
    handler$trainFoldFiles = trainFoldFiles
    handler$testFoldFiles = testFoldFiles
  }
...
#'
#' @examples
genDataFileNames = function(genoHandler,
                            #workPath = NULL,
                            onlyFold=-1,
                            path2plink="~/genoml-core-master/otherPackages/",
                            which.to.create=c("train","test")){

  workPath = dirname(genoHandler$pheno)
  if(onlyFold > 0)
    indexes = onlyFold
  else
  ...
  for (i in indexes){
    ...
    for(tt in which.to.create){
      key = paste0(tt,"FoldFiles")
      dirFold = paste0(workPath,"/","Fold",tt,i,"/")
      dir.create(dirFold, showWarnings = FALSE)

      splitIdx <- paste0(tt,i)
      #dataframe with the covariates of the individuals given by this fold
      covsFoldDT <- genoHandler$covsDat[genoHandler[[paste0(tt,"Folds")]][[splitIdx]],]
      ...
      write.table(genoHandler$covsDat[genoHandler[[paste0(tt,"Folds")]][[splitIdx]],
                                      c(genoHandler$fid,genoHandler$id)],
                  idsfile,
                  col.names=F,sep = " ", row.names = FALSE, quote = FALSE)
      foldFiles$idsFile = idsfile

      #Generating the phenotype files
      phenoFoldDT <- genoHandler$phenoDat[genoHandler[[paste0(tt,"Folds")]][[splitIdx]],]
...
#'
#' @examples
genDataFromHandler = function(genoHandler,
                              onlyFold=-1,
                              which.to.create=c("train","test"),
                              lazy=F){

  if(!is.null(genoHandler$father))
    genDataFromHandler(genoHandler$father,onlyFold,which.to.create,lazy)

  if(!is.null(genoHandler$nfolds)){
    if(onlyFold > 0)
      indexes = onlyFold
    else
      indexes = 1:genoHandler$nfolds
    ...
    for (i in indexes){
      for(tt in which.to.create){
        ...
        print(tt)
        key = paste0(tt,"FoldFiles")
        if(!is.null(genoHandler[[key]][[i]])){
          if(lazy){
            if(!file.exists(paste0(genoHandler[[key]][[i]]$genoFile,".bim"))){
              cat("Creating genotype data for fold",i,"and",tt,"with command",
                  genoHandler[[key]][[i]]$genoFileCommand,"\n")
              r = mySystem(genoHandler[[key]][[i]]$genoFileCommand)
              genoHandler[[key]][[i]]$filesCreated = T
            }else
              cat(genoHandler[[key]][[i]]$genoFile,"already created, skipping\n")
          }else{
            if(!is.null(genoHandler[[key]][[i]]$genoFileCommand)){
              cat("Creating genotype data for fold",i,"and",tt,"with command",
                  genoHandler[[key]][[i]]$genoFileCommand,"\n")
              r = mySystem(genoHandler[[key]][[i]]$genoFileCommand)
              genoHandler[[key]][[i]]$filesCreated = T
...
mostRelevantSNPs = function(handler,
  ...
  if(reduce == "PRUNE" || reduce == "DEFAULT"){
    ...
    mySystem(command)
    command = paste0(path2plink,"plink --bfile ",path2Genotype,"/",geno," --extract ",fprefix,
                     ".temp.prune.in --recode A --out ",fprefix,".reduced_genos")
    ...
    cat("The number of starting SNPs is ",nsnps,"\n")
    sbluplambda = nsnps * (1/herit) - 1
    cat("We will use a sblup-lambda parameter of",sbluplambda,"\n")

    if(force | (!file.exists(paste0(workPath,"/",geno,".forSblup.bim")) |
                !file.exists(paste0(workPath,"/",geno,".forSblup.fam")) |
                !file.exists(paste0(workPath,"/",geno,".forSblup.bed")))){
      command = paste0(path2plink,"plink --bfile ",path2Genotype,"/",geno," --pheno ",handler$pheno,".pheno --make-bed --out ",
    ...
    command = paste0(path2gcta64,"/gcta64 --bfile ",workPath,"/",geno,".forSblup --cojo-file ",path2GWAS,"/",gwas," --cojo-sblup ",sbluplambda,
                     " --cojo-wind 10000 --maf 0.01 --chr 22 --thread-num ",cores," --out ",fprefix,".temp")
    mySystem(command)
    ...
    mySystem(command)
    handler$rgenosSnpList = paste0(fprefix,".reduced_genos_snpList")
    handler$snpsToPull = paste0(fprefix,".temp.snpsToPull2")
    handler$snpsClumped = paste0(fprefix,".tempClumps.clumped")
    return(handler)
  ...
  for(fold in 1:handler$nfolds){
    for(mode in modes){
      if(mode == "train" & length(modes) == 1){
        ...
        geno = basename(handler$trainFoldFiles[[fold]]$genoFile)
        pheno = basename(gsub(".pheno","",handler$trainFoldFiles[[fold]]$phenoFile))
        workPath=paste0(dirname(handler$trainFoldFiles[[fold]]$genoFile),"/")
        path2Genotype=workPath
        cov = basename(gsub(".cov","",handler$trainFoldFiles[[fold]]$covsFile))
        prefix <- paste("g",geno,"p",pheno,"c",cov,"a",addit, sep = "-")

      }else if(mode == "test" & length(modes) > 1){
        geno = basename(handler$testFoldFiles[[fold]]$genoFile)
        pheno = basename(gsub(".pheno","",handler$testFoldFiles[[fold]]$phenoFile))
        workPath=paste0(dirname(handler$testFoldFiles[[fold]]$genoFile),"/")
        path2Genotype=workPath
        cov = basename(gsub(".cov","",handler$testFoldFiles[[fold]]$covsFile))

        if(handler$snpsToPull == fsHandler$snpsToPull){
          #We need the previous prefix
          genotrn = basename(handler$trainFoldFiles[[fold]]$genoFile)
          phenotrn = basename(gsub(".pheno","",handler$trainFoldFiles[[fold]]$phenoFile))
          covtrn = basename(gsub(".cov","",handler$trainFoldFiles[[fold]]$covsFile))
          workPathtrn=paste0(dirname(handler$trainFoldFiles[[fold]]$genoFile),"/")
      ...
      handler[[paste0(mode,fold,"mldata")]] = fname
...
trainAndTest = function(handler,
  ...
  stopifnot(handler$plan == "holdout" | handler$plan == "k-fold cv")

  if(handler$plan == "holdout"){
    newhandler = NULL
    newhandler$nfolds = 1
    ...
    for(i in 1:handler$nfolds){
      newhandler = NULL
      newhandler$nfolds = 1
      print(handler$trainFoldFiles)
      newhandler$trainmldata = handler$trainFoldFiles[[paste0("train",i)]]
